...
Both on and off-target tests are important. For on-target tests, see **stub web-peripheral-interface post**. This post covers off-target tests using the GoogleTest framework.
...
- `main/hal` - the Hardware Abstraction Layer which defines hardware behavior that is consistent across target platforms
- `main/esp32` - any actual ESP32 library calls
...
![Unit test architecture stub](/assets/img/rover/unit-test-architecture-stub.png)
...
What `tests/CMakeLists.txt` does is:
...
![Local test runner stub](/assets/img/rover/local-test-runner-stub.png)
...
Both a clang formatter and the unit test executeable are wired into `.pre-commit-config.yaml`. I am a lazy person; if it is hard to push code that is unformatted and untested, I tend to do more formatting and testing.

![Pre-commit yaml stub](/assets/img/rover/pre-commit-yaml-stub.png)
...
![Pre-commit hook stub](/assets/img/rover/pre-commit-hook-stub.png)
...
`HttpServerInterface` covers the routing logic and the helper functions for parsing key-value pairs from URI requests. String parsing and callback logic should be unit tested because it is bug-prone and does not require ESP32 hardware to validate.

`HttpServerESP32` contains the code needed to implement `HttpServerInterface` on an actual ESP32 device. These function calls can not be tested locally (atleast, not easily - high-end chips tend to ship with simulators, but the $20 ESP32 has no lead time and why simulate when you can just buy one).
